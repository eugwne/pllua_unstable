...
static int luaP_rowsaux (lua_State *L) {
    ...
    if (SPI_processed > 0) { /* any row? */
        if (!init) { /* register tupdesc */
            c->data = tsk_object_new(&datatuptable_def_t, L, SPI_tuptable->tupdesc);
...
static int luaP_tupleindex (lua_State *L) {
    ...
    if (i >= 0) {
        if (t->changed == -1) { /* read-only? */
            //need to fix, if shared[..] = tuple, t->desc might be dead...
            bool isnull;
        ...
        elog(ERROR, "[pllua]: 'tuple' object has no attribute '%s'", name);
        lua_pushnil(L);
    }
    return 1;
}
